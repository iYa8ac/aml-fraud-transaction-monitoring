...
namespace Jube.Data.Repository
{
    ...
    {
        public async Task<ExhaustiveSearchInstancePromotedTrialInstanceSensitivity> InsertAsync(
            ExhaustiveSearchInstancePromotedTrialInstanceSensitivity model, CancellationToken token = default)
        {
            model.Id = await dbContext.InsertWithInt32IdentityAsync(model, token: token).ConfigureAwait(false);
            return model;
        }

        public async Task<IEnumerable<ExhaustiveSearchInstancePromotedTrialInstanceSensitivity>>
        ...
        {
            return await dbContext.ExhaustiveSearchInstancePromotedTrialInstanceSensitivity
