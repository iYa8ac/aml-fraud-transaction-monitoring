/* Copyright (C) 2022-present Jube Holdings Limited.
 *
 * This file is part of Jube™ software.
 *
 * Jube™ is free software: you can redistribute it and/or modify it under the terms of the GNU Affero General Public License
 * as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.
 * Jube™ is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty
 * of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Affero General Public License for more details.

 * You should have received a copy of the GNU Affero General Public License along with Jube™. If not,
 * see <https://www.gnu.org/licenses/>.
 */

namespace Jube.Engine.BackgroundTasks.TaskStarters
{
    using System;
    using System.Threading.Tasks;
    using Case;
    using Context;

    public class CaseCreationTaskStarter(Context context)
    {
        public async Task StartAsync()
        {
            while (true)
            {
                try
                {
                    lock (context.ConcurrentQueues.PendingCases)
                    {
                        if (context.ConcurrentQueues.PendingCases.IsEmpty)
                        {
                            context.Services.TaskCoordinator.CancellationToken.ThrowIfCancellationRequested();
                        }
                    }

                    if (!context.ConcurrentQueues.PendingCases.TryDequeue(out var createCase))
                    {
                        await Task.Delay(100);
                        
                        continue;
                    }

                    await CaseProcessing.CreateAsync(context.Services.DynamicEnvironment,
                        createCase, context.Services.Log);

                    // ReSharper disable once RedundantAssignment
                    createCase = null;
                }
                catch (OperationCanceledException ex)
                {
                    context.Services.Log.Info($"Graceful Cancellation CreateCaseTaskStarter: has produced an error {ex}");

                    break;
                }
                catch (Exception ex)
                {
                    context.Services.Log.Error($"Case Creation: Error {ex}");

                    // ReSharper disable once MethodSupportsCancellation
                }

                await Task.Delay(100).ConfigureAwait(false);
            }
        }
    }
}
